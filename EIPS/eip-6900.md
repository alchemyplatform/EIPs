...
`StandardExecutor` are mainly used for open-ended executions.

`PluginExecutor` are specifically used by plugins to request account to execute on account's context. Explicit permissions are required for plugins to use `PluginExecutor`.
...
- Plugin execution functions can only request access to `PluginExecutor`.
...
#### Calls to `onHookApply` and `onHookUnapply` on plugins

Plugins may have hooks that can be used by other plugins. Two convinient optional functions (`onHookApply` and `onHookUnapply`) are provided by the hook-supplying plugins. The goals of those functions are:

- To help hook-supplying plugins to guard and setup new hook function request from other plugins. Depends on the nature of the plugin and the hook, `onHookApply` might be critical to call before a hook can be used on a different plugin.
- To help hook-requesting plugins to initiate the usage of a certain hook from the hook-supplying plugins.

An optional data field can be used to communicate between the two plugins to define granular configurations or parameters.
